...
uni.lobo = {}; //配置全局url
uni.lobo.env = 'pro';if (uni.lobo.env === 'dev') {uni.lobo.baseURL = 'http://127.0.0.1:8088';} else {uni.lobo.baseURL = '';}uni.lobo.request = /*#__PURE__*/function () {var _ref2 = _asyncToGenerator( /*#__PURE__*/_regenerator.default.mark(function _callee(_ref) {var url, method, data, query, header, result, token;return _regenerator.default.wrap(function _callee$(_context) {while (1) {switch (_context.prev = _context.next) {case 0:url = _ref.url, method = _ref.method, data = _ref.data, query = _ref.query, header = _ref.header;result = {}; //获取token
            token = uni.getStorageSync('token'); //判断是不是本地开发测试
            if (!(uni.lobo.env === 'dev')) {_context.next = 9;break;}_context.next = 6;return new Promise(function (resolve, reject) {uni.request({ url: uni.lobo.baseURL + url + (query ? '?' + uni.lobo.urlEndcode(query) : ''), method: method, data: data, header: { token: token }, success: function success(res) {//响应拦截
                  if (res.data.code === 201) {_index.default.commit('userStore/muSetToken', '');uni.switchTab({ url: '/pages/user/user' });resolve({});} else {resolve(res.data);}} });});case 6:result = _context.sent;_context.next = 14;break;case 9:_context.next = 11;return wx.cloud.callContainer({ "config": { "env": "prod-8gmhhtejbb2f1d96" }, "path": url + (query ? '?' + uni.lobo.urlEndcode(query) : ''), "header": { "X-WX-SERVICE": "lobo-shop", "content-type": "application/json", "token": token }, data: data, "method": method });case 11:result = _context.sent; //响应拦截
            //登陆超时
            result = result.data;if (result.code === 201) {_index.default.commit('userStore/muSetToken', '');uni.showToast({ title: '请先登陆', icon: 'error', duration: 1500 });setTimeout(function () {uni.switchTab({ url: '/pages/user/user' });}, 500);}case 14:return _context.abrupt("return", result);case 15:case "end":return _context.stop();}}}, _callee);}));return function (_x) {return _ref2.apply(this, arguments);};}(); //query参数urlEncode
